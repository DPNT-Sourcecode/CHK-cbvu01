export default class CheckoutSolution {
  checkout(skus) {
    ...
    const multi = {
      A: [{ qty: 5, price: 200 }, { qty: 3, price: 130 }],
      B: [{ qty: 2, price: 45 }],
      H: [{ qty: 10, price: 80 }, { qty: 5, price: 45 }],
      K: [{ qty: 2, price: 120 }],
      P: [{ qty: 5, price: 200 }],
      Q: [{ qty: 3, price: 80 }],
      V: [{ qty: 3, price: 130 }, { qty: 2, price: 90 }],
    };
    ...
    let groupCount = 0;
    for (const sku of groupSkus) groupCount += counts[sku] || 0;

    const groups = Math.floor(groupCount / 3);
    let total = groups * 45;

    let toConsume = groups * 3;
    if (toConsume > 0) {
      const skusByPriceDesc = [...groupSkus].sort((a, b) => prices[b] - prices[a]);
